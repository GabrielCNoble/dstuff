...
struct ds_heap_t ds_create_heap(uint32_t size)
{
    ...
    if(size)
    {
        heap.alloc_chunks = create_stack_list(sizeof(struct ds_chunk_t), 128);
        heap.free_chunks = create_list(sizeof(struct ds_chunk_t), 128);
        heap.size = size;
...
void ds_reset_heap(struct ds_heap_t *heap)
{
    if(heap && heap->size)
    {
        ...
        struct ds_chunk_t *chunk = get_list_element(&heap->free_chunks, 0);
        chunk->start = 0;
        chunk->align = 0;
...
void ds_defrag_heap(struct ds_heap_t *heap)
{
    ...
    if(heap)
    {
        ...
        qsort_list(&heap->free_chunks, ds_cmp_chunks);
