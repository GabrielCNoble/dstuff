...
struct dbvh_tree_t create_dbvh_tree(uint32_t used_data_size);
...
struct dbvh_tree_t create_dbvh_tree(uint32_t user_data_size)
{
    struct dbvh_tree_t tree;

    tree.root = INVALID_DBVH_NODE_INDEX;
    tree.node_pool = create_stack_list(sizeof(struct dbvh_node_t) + user_data_size, 128);
...
uint32_t alloc_dbvh_node(struct dbvh_tree_t *tree)
{
    ...
    node->user_data = (char *)node + sizeof(struct dbvh_node_t);
